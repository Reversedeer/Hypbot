...
class Consts:
    BUFFER_SIZE = 1048576
    # 发送私聊消息的服务器地址。
ADDRESS_SEND_HOST = '127.0.0.1:8081'
...
# 匹配用户名、UUID、QQ号码、公会名称、换行符和样式代码的正则表达式常量。
    ...
    def get_level(exp, levels):
        ...
        *levels, last_level = levels
        ...
        if exp >= levels_total_exp:
            return int(level_count + (exp - levels_total_exp) // last_level[0]), int(
                (exp - levels_total_exp) % last_level[0]), last_level[1]
        else:
            ret_level = 0
            ret_exp = exp
            ret_full_exp = '0'
...
class Message:
    def __init__(self, main, message=None, group=-1, user=10001, id=0, raw_message=''):
        ...
        self.data = {
            'prefix': main.options.get(f'global.prefixes.{self.group}',
                                       main.options.get(f'global.prefixes.default', '/')),
...
class Main:
    def __init__(self):
        ...
        self.options = Options('config.json', {
            'lists': {
                'global': {
                    'group_default': 'whitelist',
                    'group_whitelist': [],
            ...
            'global': {
                'apisets': {
                    'default': {
                        ...
                        'hypixel_apikey': '43c7e1da-53ec-4291-8c11-6bc7fba396ab',
                        ...
                        'antisniper_apikey': '7d51d02d-439b-4668-92df-24b99e006933',
        ...
        self.bot_map = {}
        self.register_bot(BotCommand('admin', ('',), (), lambda main, message:
        main.command(message.message[2:], message.group, message.user)[1]))
        ...
        self.register_bot(BotCommand('luck', ('luck', 'yluck'), (Pipes.cooldown,), lambda main, message: Utils.last(
            ...
            message.set('luck.from', main.options.get(f'options.luck.{message.user}',
                                                      main.options.get(f'options.luck.default', [0, 100]))[0]),
            message.set('luck.to', main.options.get(f'options.luck.{message.user}',
                                                    main.options.get(f'options.luck.default', [0, 100]))[1]),
        ...
        self.register_bot(BotCommand('mcname', ('mc', 'minecraft', 'profile', 'skin', 'mcskin'), (
        Pipes.cooldown, APIs.RE_USERNAME, APIs.MOJANG_PROFILE, APIs.MOJANG_SESSION_NAME, Pipes.session),
                                     message_profile))
        self.register_bot(BotCommand('mcuuid', ('mcuuid', 'uuid'),
                                     (Pipes.cooldown, APIs.RE_UUID, APIs.MOJANG_SESSION_UUID, Pipes.session),
                                     message_profile))
        self.register_bot(BotCommand('hypixelinternal', ('__hypixelinternal',), (Pipes.cooldown, APIs.RE_USERNAME,
                                                                                 APIs.MOJANG_PROFILE,
                                                                                 APIs.HYPIXEL_PLAYER, Pipes.hypixel,
                                                                                 Pipes.command_hypixel,
                                                                                 Pipes.replace(
                                                                                     {'[': 'api_hypixel_player.player.',
                                                                                      '(': 'api_hypixel_player.player.achievements.general_'})
                                                                                 ), lambda main, message: '''
            %s 的 Hypixel 信息:
            等级: %s | 人品: %s%s
            成就点数: %s | 赠送 Rank: %s
            ...
            最近游玩: %s%s
        ''' % Utils.format(main, message,
                           'hypixel.name?', 'hypixel.level', '[karma', 'hypixel.rank_color?', '[achievementPoints',
                           '[giftingMeta.ranksGiven',
                           '(quest_master', '(challenger', '(wins', '(coins', '[seasonal.silver',
                           '[tourney.total_tributes',
                           '[userLanguage^?', '[firstLogin*?', '[lastLogin*?', '[lastLogout*?', '[mostRecentGameType^?',
                           'hypixel.name_change?'
                           )))
        self.register_bot(BotCommand('apikey', ('api', 'key', 'apikey'), (Pipes.cooldown,
                                                                          APIs.HYPIXEL_API,
                                                                          APIs.MOJANG_SESSION_HYPIXEL_API,
                                                                          Pipes.replace({'[': 'api_hypixel_key.record.',
                                                                                         '(': 'api_mojang_session.'})
                                                                          ), lambda main, message: '''
            %s 的 API Key 信息:
            UUID: %s
        ...
        self.register_bot(BotCommand('ban', ('ban', 'bans', 'punishment', 'punishments'), (Pipes.cooldown,
                                                                                           APIs.HYPIXEL_PUNISHMENTSTATS,
                                                                                           Pipes.replace({
                                                                                                             '[': 'api_hypixel_punishmentstats.'})
                                                                                           ), lambda main, message: '''
            Hypixel 封禁信息:
            Watchdog 封禁:
            - 一分钟内: %s
            ...
            - 今日封禁: %s
            - 总封禁: %s
        ''' % Utils.format(main, message, '[watchdog_lastMinute', '[watchdog_rollingDaily', '[watchdog_total',
                           '[staff_rollingDaily', '[staff_total')))
        self.register_bot(BotCommand('bedwars', ('bw', 'bedwar', 'bedwars'), (Pipes.cooldown, APIs.RE_USERNAME,
                                                                              APIs.MOJANG_PROFILE, APIs.HYPIXEL_PLAYER,
                                                                              Pipes.hypixel, Pipes.command_bedwars,
                                                                              Pipes.game_mode(Maps.MODE_BEDWARS,
                                                                                              'bedwars.mode'),
                                                                              Pipes.replace({
            ...
            收集钻石: %s | 收集绿宝石: %s%s
        ''' % Utils.format(main, message,
                           'bedwars.level?', 'bedwars.star?', 'hypixel.name?', 'bedwars.mode?', 'bedwars.exp',
                           'bedwars.full_exp?',
        ...
        self.register_bot(BotCommand('skywars', ('sw', 'skywar', 'skywars'), (Pipes.cooldown, APIs.RE_USERNAME,
                                                                              APIs.MOJANG_PROFILE, APIs.HYPIXEL_PLAYER,
                                                                              Pipes.hypixel, Pipes.command_skywars,
                                                                              Pipes.replace({
                                                                                                '[': 'api_hypixel_player.player.stats.SkyWars.'})
                                                                              ), lambda main, message: '''
            [%s%s] %s 的空岛战争数据:
            ...
            游戏时长: %s%s
        ''' % Utils.format(main, message,
                           'skywars.level?', 'skywars.star?', 'hypixel.name?', 'skywars.exp', 'skywars.full_exp?',
                           '[opals', '[shard',
                           '[coins', '[cosmetic_tokens', 'skywars.corrupt?', '[kills', '[deaths', '/',
                           '[wins', '[losses', '/', '[souls', '[heads', '[assists', '[time_played&',
                           'hypixel.name_change?'
                           )))
        self.register_bot(BotCommand('megawalls', ('mw', 'megawall', 'megawalls'), (Pipes.cooldown, APIs.RE_USERNAME,
                                                                                    ...
                                                                                    APIs.HYPIXEL_PLAYER, Pipes.hypixel,
                                                                                    Pipes.replace({
                                                                                                      '[': 'api_hypixel_player.player.stats.Walls3.'})
                                                                                    ), lambda main, message: '''
            %s 的超级战墙数据:
            硬币: %s | 凋灵伤害: %s
            击杀: %s | 死亡: %s | K/D: %s
            ...
            助攻: %s | 最终助攻: %s%s
        ''' % Utils.format(main, message,
                           'hypixel.name?', '[coins', '[wither_damage', '[kills', '[deaths', '/', '[final_kills',
                           '[final_deaths', '/',
        ...
        self.register_bot(BotCommand('duels', ('duel', 'duels'), (Pipes.cooldown, APIs.RE_USERNAME,
                                                                  APIs.MOJANG_PROFILE, APIs.HYPIXEL_PLAYER,
                                                                  Pipes.hypixel,
                                                                  Pipes.game_mode(Maps.MODE_DUELS, 'duels.mode'),
                                                                  Pipes.replace(
                                                                      {'[': 'api_hypixel_player.player.stats.Duels.'})
                                                                  ), lambda main, message: '''
            %s 的决斗游戏%s数据:
            硬币: %s | 回合数: %s
            近战命中率: %s%% | 射击命中率: %s%%
        ...
        ''' % Utils.format(main, message,
                           ...
                           '%',
                           '?!bow_hits', '?!bow_shots', '%', '`kills', '`deaths', '/', '!wins', '!losses', '/',
                           'hypixel.name_change?'
                           )))
        self.register_bot(BotCommand('blitz', ('bsg', 'blitz'), (Pipes.cooldown, APIs.RE_USERNAME,
                                                                 APIs.MOJANG_PROFILE, APIs.HYPIXEL_PLAYER,
                                                                 Pipes.hypixel, Pipes.plus(
            'api_hypixel_player.player.stats.HungerGames.wins', 'api_hypixel_player.player.stats.HungerGames.deaths',
            'blitz.games'
        ...
        self.register_bot(BotCommand('uhc', ('uhc',), (Pipes.cooldown, APIs.RE_USERNAME,
                                                       APIs.MOJANG_PROFILE, APIs.HYPIXEL_PLAYER, Pipes.hypixel,
                                                       Pipes.plus(
                                                           'api_hypixel_player.player.stats.UHC.deaths',
                                                           'api_hypixel_player.player.stats.UHC.deaths_solo',
                                                           'uhc.deaths'
                                                       ), Pipes.replace(
            {'[': 'api_hypixel_player.player.stats.UHC.', '(': 'api_hypixel_player.player.achievements.uhc_'})
                                                       ), lambda main, message: '''
            %s 的 UHC 数据:
            硬币: %s | 分数: %s
            击杀: %s | 死亡: %s | K/D: %s
        ...
        self.register_bot(BotCommand('mm', ('mm', 'murder'), (Pipes.cooldown, APIs.RE_USERNAME,
                                                              APIs.MOJANG_PROFILE, APIs.HYPIXEL_PLAYER, Pipes.hypixel,
                                                              Pipes.replace({
                                                                                '[': 'api_hypixel_player.player.stats.MurderMystery.'})
                                                              ), lambda main, message: '''
            %s 的密室杀手数据:
            硬币: %s | 胜场: %s | 场数: %s
            击杀: %s | 死亡: %s | K/D: %s
        ...
        self.register_bot(BotCommand('tnt', ('tnt', 'tntgame', 'tntgames'), (Pipes.cooldown, APIs.RE_USERNAME,
                                                                             APIs.MOJANG_PROFILE, APIs.HYPIXEL_PLAYER,
                                                                             Pipes.hypixel, Pipes.replace(
            {'[': 'api_hypixel_player.player.stats.TNTGames.'})
                                                                             ), lambda main, message: '''
            %s 的掘战游戏数据:
            硬币: %s
            方块掘战: 纪录: %s | 胜场: %s
            ...
            - 击杀: %s | 死亡: %s | K/D: %s%s
        ''' % Utils.format(main, message,
                           'hypixel.name?', '[coins', '[record_tntrun&?', '[wins_tntrun', '[record_pvprun&?',
                           '[wins_pvprun',
        ...
        self.register_bot(BotCommand('pit', ('pit',), (Pipes.cooldown, APIs.RE_USERNAME,
                                                       APIs.MOJANG_PROFILE, APIs.HYPIXEL_PLAYER, Pipes.hypixel,
                                                       Pipes.plus(
                                                           'api_hypixel_player.player.stats.Pit.pit_stats_ptl.kills',
                                                           'api_hypixel_player.player.stats.Pit.pit_stats_ptl.assists',
                                                           'pit.kill_and_assist'
                                                       ), Pipes.replace(
            {'[': 'api_hypixel_player.player.stats.Pit.pit_stats_ptl.',
             '(': 'api_hypixel_player.player.stats.Pit.profile.'})
                                                       ), lambda main, message: '''
            %s 的天坑之战数据:
            金币: %s | 最高连杀: %s
            击杀: %s | 死亡: %s | K/D: %s
        ...
        ''' % Utils.format(main, message,
                           ...
                           '[tag?', 'guild.tag_color?', 'guild.double_exp', 'guild.double_coins', '[publiclyListed$',
                           'guild.preferred_games?'
                           )
        self.register_bot(BotCommand('guildname', ('gname', 'guildname'), (Pipes.cooldown,
                                                                           lambda main, message: message.set(
                                                                               'guild.name',
                                                                               ' '.join(message.args[1:])),
                                                                           APIs.RE_GUILDNAME,
                                                                           APIs.HYPIXEL_GUILD_NAME,
                                                                           Pipes.command_guild('name'),
                                                                           Pipes.command_guild_player('name'),
                                                                           APIs.MOJANG_SESSION_HYPIXEL_GUILDNAME,
                                                                           Pipes.replace(
                                                                               {'[': 'api_hypixel_guild_name.guild.'})
                                                                           ),
        ...
        self.register_bot(BotCommand('guild', ('g', 'guild'), (Pipes.cooldown, APIs.RE_USERNAME,
                                                               APIs.MOJANG_PROFILE, APIs.HYPIXEL_GUILD_PLAYER,
                                                               Pipes.command_guild('player'),
                                                               Pipes.command_guild_player('player'),
        ...
        self.register_bot(BotCommand('bwshop', ('bwshop', 'bwfav'), (Pipes.cooldown, APIs.RE_USERNAME,
            ...
            物品栏:%s%s
        ''' % Utils.format(main, message,
                           'bedwars.level?', 'bedwars.star?', 'hypixel.name?', 'bedwars.shop?', 'bedwars.slots?',
                           'hypixel.name_change?'
                           )))
        self.register_bot(BotCommand('denick', ('denick',), (Pipes.cooldown, APIs.RE_USERNAME,
                                                             APIs.ANTISNIPER_DENICK,
                                                             Pipes.replace({'[': 'api_antisniper_denick.player.'})
                                                             ), lambda main, message: '''
            %s -> %s
            查询的 Nick: %s
            Denick 时间: %s
            上次发现: %s
            (数据来自 Antisniper)
        ''' % Utils.format(main, message, '[ign?', '[latest_nick?', '[queried_nick?', '[first_detected~?',
                           '[last_seen~?')))
        self.register_bot(BotCommand('findnick', ('findnick',), (Pipes.cooldown, APIs.RE_USERNAME,
                                                                 APIs.ANTISNIPER_FINDNICK,
                                                                 Pipes.replace({'[': 'api_antisniper_findnick.player.'})
                                                                 ), lambda main, message: '''
            %s -> %s
            Denick 时间: %s
            上次发现: %s
        ...
        self.register_bot(BotCommand('winstreak', ('ws', 'winstreak'), (Pipes.cooldown, APIs.RE_USERNAME,
            ...
            {'(': 'api_antisniper_winstreak.player.', '[': 'api_antisniper_winstreak.player.data.', ']': '_winstreak'})
                                                                        ), lambda main, message: '''
            %s 的起床战争连胜数据:
            总连胜: %s
            单人模式: %s
            ...
            (4v4: %s)
            (数据来自 Antisniper)
        ''' % Utils.format(main, message, '(ign?', '[overall]?', '[eight_one]?', '[eight_two]?', '[four_three]?',
                           '[four_four]?', '[two_four]?')))
        self.register_bot(BotCommand('winstreakall', ('wsall', 'winstreakall'), (Pipes.cooldown, APIs.RE_USERNAME,
                                                                                 APIs.ANTISNIPER_WINSTREAK,
                                                                                 Pipes.replace({
        ...
        self.register_bot(BotCommand('winstreakhypixel', ('wshyp',), (Pipes.cooldown, APIs.RE_USERNAME,
            ...
            (4v4: %s)
            (数据来自 Hypixel)
        ''' % Utils.format(main, message, '(displayname?', '[winstreak?', '[eight_one]?', '[eight_two]?',
        ...
        self.register_bot(BotCommand('hypixel', ('hyp', 'hypixel'), (APIs.RE_USERNAME,
                                     ...
                                     message.get(f'commands.{message.get("args.2", "__hypixelinternal")}',
                                                 BotError(BotException('未知的指令!'))).process(main, Message(main,
                                                                                                              group=message.group,
                                                                                                              user=message.user,
                                                                                                              id=message.id,
                                                                                                              raw_message=
        ...
        self.register_bot(BotCommand('optifinecape', ('ofcape', 'optifine'), (Pipes.cooldown, APIs.RE_USERNAME,
                                                                              APIs.MOJANG_PROFILE, APIs.OPTIFINE_CAPE,
                                                                              APIs.OPTIFINE_FORMAT,
                                                                              Pipes.command_optifine_cape,
                                                                              ), lambda main, message: '''
            %s 的 OptiFine 披风信息:
            Design: %s%s
            Top: %s
            Bottom: %s%s
        ''' % Utils.format(main, message,
                           'api_mojang_profile.name?', 'ofcape.design?', 'ofcape.banner?', 'ofcape.top?',
                           'ofcape.bottom?', 'ofcape.custom?'
                           )))
        self.register_bot(
            BotCommand('qqapi', ('qqapi',), (Pipes.cooldown, APIs.RE_QQNUMBER, APIs.QQAPI_QQAPI), lambda main, message: '''
            %s 的 QQ 账号信息:
            手机号: %s
            地区: %s
        ''' % Utils.format(main, message, 'api_qqapi_qqapi.qq?', 'api_qqapi_qqapi.phone?',
                           'api_qqapi_qqapi.phonediqu?')))
    ...
    def process(self, message):
        try:
            if isinstance(self.record_mode, tuple) and self.record_mode == (
            message.group, message.user) and self.record_to is not None:
    ...
    def command(self, command_line, group, user):
        try:
            ...
            if command == 'quit':
                ...
                self.options.set(eval(args[0]),
                                 self.options.get(eval(args[1]) if len(args) >= 2 else self.record_messages['']))
